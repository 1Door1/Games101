...
cv::Point2f recursive_bezier(const std::vector<cv::Point2f> &control_points, float t) 
{
    ...
    {
        NextRecursive.push_back(cv::Point2f(control_points.at(i).x*(1-t)+t*control_points.at(i+1).x,
...
void bezier(const std::vector<cv::Point2f> &control_points, cv::Mat &window) 
{
    ...
    for(double t = 0.0;t <= 1.0;t+= 0.001)
    {
        ...
        window.at<cv::Vec3b>(point.y,point.x)[1]=std::min(255,colorValue+window.at<cv::Vec3b>(point.y,point.x)[1]);
    }
}
